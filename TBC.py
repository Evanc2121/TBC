 ...
 armor: {self.armor}""")
    ...
    def hit(self, enemy):
        ...
        if hitRand <= self.hitChance:
            print(f"{self.name} hits {enemy.name} for {dmgNum} points of damage, {enemy.name}'s armor absorbs {enemy.armor} points of damage")
            if dmgNum < enemy.armor:
                dealtDmg = dmgNum - enemy.armor
                if enemy.hitPoints - dealtDmg >= 0:
                    enemy.hitPoints = enemy.hitPoints - dealtDmg
                else:
                    enemy.hitPoints = 0
